...
export const cleanSitemapContent = (sitemapContent: string): string => {
  ...
  if (!cleanedContent.startsWith('<?xml')) {
    ...
    if (xmlDeclarationMatch) {
      ...
      // Add it to the beginning with no whitespace before it
      cleanedContent = '<?xml version="1.0" encoding="UTF-8"?>' + cleanedContent.trim();
    } else {
...
export const ensureCorrectSitemapStart = (content: string): string => {
  ...
  if (!cleanContent.startsWith(xmlDeclaration)) {
    ...
    if (xmlDeclMatch) {
      ...
      cleanContent = xmlDeclaration + cleanContent.trim();
    } else {
...
export const normalizeSitemapXml = (sitemapContent: string): string => {
  ...
  // Ensure it starts with the correct XML declaration (with no whitespace before)
  const xmlDeclaration = '<?xml version="1.0" encoding="UTF-8"?>';
  if (!normalized.startsWith(xmlDeclaration)) {
    ...
    normalized = xmlDeclaration + normalized.trim();
  }
...
export const debugSitemapStructure = (sitemapContent: string): void => {
  ...
  console.log('First 100 chars:', sitemapContent.substring(0, 100).replace(/\n/g, '\\n'));
  console.log('Has XML declaration:', sitemapContent.includes('<?xml'));
  console.log('XML declaration at start:', sitemapContent.trim().startsWith('<?xml'));
